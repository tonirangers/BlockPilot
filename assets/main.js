/* assets/main.js - BlockPilot Pro (Mode Alpha Pur) */
(async () => {
  ...
  const I18N = {
    fr: {
      ...
      chartLabel: "Surplus généré par BlockPilot (Alpha)",
      chartSub: "Valeur ajoutée par rapport au Holding passif"
    },
    en: {
      menu: "Menu", close: "Close",
      priceHint: (px, unit, scn) => `Ref price: ~$${px}/${unit}. Scenario: ${scn ? "+" + (scn*100).toFixed(0) + "%" : "stable"}.`,
      chartLabel: "Surplus generated by BlockPilot (Alpha)",
      chartSub: "Value added vs Passive Holding"
    }
  };
      ...
      options: {
        ...
        scales: {
          ...
          y: { 
            ...
            ticks: { callback: (v)=> '+$' + (v/1).toLocaleString(), color: '#64748B' } 
          }
        },
  ...
  function updateChartData(chart, principalUSD, apr, priceScenario, duration, asset, prices) {
    ...
    const labels = [];
    const dataAlpha = []; // Juste le surplus
  ...
  function initCalc(cfg, pricesUSD, yields) {
    ...
    let scenario = 0.2;
    let durationYears = 3; // Default 3y
